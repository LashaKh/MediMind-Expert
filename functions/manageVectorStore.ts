...
async function handleVectorStoreRequest(event: HandlerEvent, context: any): Promise<HandlerResponse> {
...
export const handler = withMedicalSecurity(
  async (event: HandlerEvent, context: any): Promise<HandlerResponse> => {
    try {
      return await handleVectorStoreRequest(event, context);
    } catch (error) {
