...
class OfflineActionQueue {
  ...
  public enqueue(type: string, data: any): string {
  ...
  public async processQueue() {
    ...
    for (const action of actionsToProcess) {
      try {
        ...
        console.log(`Successfully processed queued action: ${action.type}`, action);
      } catch (error) {
        ...
        if (action.retryCount < this.maxRetries) {
          ...
          console.error(`Max retries exceeded for action: ${action.type}`, action);
        }
      }
    }
...
export const useOfflineQueue = () => {
  ...
  const enqueueAction = React.useCallback((type: string, data: any) => {
