...
const PodcastTracker: React.FC<PodcastTrackerProps> = ({ podcastId, onComplete }) => {
  ...
  const checkPodcastStatus = async (id: string) => {
    try {
      ...
      addLog('error', `❌ Status check error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  const updateTrackerFromStatus = (statusData: any) => {
    ...
    switch (podcast.status) {
      ...
      case 'generating':
        progress = podcast.playnote_id ? 70 : 40;
        currentStep = podcast.playnote_id 
          ? 'PlayAI processing audio...' 
          : 'Sending to PlayAI...';
        estimatedTime = 180; // 3 minutes
        addLog('info', `🎬 Generation in progress${podcast.playnote_id ? ` (ID: ${podcast.playnote_id})` : ''}`);
        break;
