...
import { Heart, Baby, AlertTriangle, Info, CheckCircle, Clock, User, Activity, Target, Stethoscope, Award, Shield } from 'lucide-react';
  ...
  const handleCalculate = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const calculationResult = calculateOBGYN('apgar-score', input) as ApgarScoreResult;
        setResult(calculationResult);
        
      } catch (error) {
        setErrors({ calculation: error instanceof Error ? error.message : t('calculators.obgyn.apgar_score.calculation_error') });
      } finally {
        setIsCalculating(false);
      }
    }, 1500);
