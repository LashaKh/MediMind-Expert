...
export class RetryableAPIClient {
  ...
  async post<T>(url: string, data?: any, options?: RequestInit & { retryConfig?: Partial<RetryConfig> }): Promise<T> {
  ...
  async put<T>(url: string, data?: any, options?: RequestInit & { retryConfig?: Partial<RetryConfig> }): Promise<T> {
    return this.request<T>(url, {
      ...options,
      method: 'PUT',
      body: data ? JSON.stringify(data) : undefined,
